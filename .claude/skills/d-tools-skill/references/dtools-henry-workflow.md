...
- keep local opportunity memory in `info/`
...
2. save snapshot in `info/<opportunity-number>.json`
3. update `info/manifest.json`

Relevant files:
- `scripts/opportunity_info_store.js`
- `scripts/sync_opportunity_info.js`

## Extra opportunity info rule

When the user gives extra details that do not belong to the D-Tools schema:

1. save them in `info/<opportunity_id>_info.json`
2. update `info/manifest.json`
...
Relevant file:
- `scripts/save_opportunity_extra_info.js`
...
1. save them in `product_info/<product_id>_info.json`
2. update `product_info/manifest.json`
...
Relevant file:
- `scripts/product_info_store.js`

## Manifest rule

`info/manifest.json` is the registry for all opportunities we have touched locally.
...
`product_info/manifest.json` is the registry for all product notes we have touched locally.
...
Use:
- `scripts/generate_quote_html.js`
...
node skills/d-tools-skill/scripts/generate_quote_html.js --quote-id <QUOTE_ID>
```
...
1. inspect `info/manifest.json`
2. inspect matching files in `info/`
3. use `scripts/dtools_api_client.js` for live API operations
