...
<head>
  ...
  <title>Quote ${escapeHtml(quote.number || quote.id)}</title>
  <style>
    :root {
      --ink: #12202f;
      --muted: #5a6b7f;
...
<body>
  <div class="page">
    <header class="topbar">
      <div>
        <h1 class="title">Quote Proposal</h1>
    ...
    <footer class="footer-note">
      This proposal page is auto-generated outside D-Tools using live D-Tools API data.
      Final acceptance and execution can be manually mirrored into D-Tools.
    </footer>
  </div>
</body>
</html>`;
...
async function main() {
  ...
  const outDir = readArg("--out-dir", "generated_quotes");
  const outFileArg = readArg("--out-file");

  if (!quoteId) {
    console.error(
      "Usage: node skills/d-tools-skill/scripts/generate_quote_html.js --quote-id <QUOTE_ID> [--out-dir generated_quotes] [--out-file filename.html]"
    );
    process.exit(1);
  }
