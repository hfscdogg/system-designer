 ...
 * - DTOOLS_API_KEY (required)
 * - DTOOLS_BASIC_AUTH (required; the "Basic ..." value from the D-Tools API docs — kept out of the repo)
 */
...
function createDToolsClient(config = {}) {
  ...
  if (!basicAuth) {
    throw new Error(
      "Missing DTOOLS_BASIC_AUTH. Set it in environment (value from the D-Tools API docs) or pass { basicAuth } to createDToolsClient."
    );
  }
